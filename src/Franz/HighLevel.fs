...
type IConsumer = 
    ...
    abstract GetPosition : unit -> PartitionOffset array
    
    /// Set the current consumer position
    abstract SetPosition : PartitionOffset seq -> unit
...
type BaseConsumer(brokerRouter : IBrokerRouter, consumerOptions : ConsumerOptions) = 
    ...
    /// Sets the current consumer offsets
    override __.SetPosition(offsets : PartitionOffset seq) = 
        offsets
    ...
    let updateConsumerPosition (partitionAssignments : PartitionAssignment array) = 
        ...
        let unavailableOffsets = 
            ...
            |> Seq.filter (fun x -> 
                   ...
                   |> not)
            |> Seq.map (fun x -> 
                   { PartitionOffset.Offset = 0L
                     PartitionOffset.PartitionId = x
                     PartitionOffset.Metadata = "" })

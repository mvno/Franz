...
type ILogger = 
    inherit IDisposable
    abstract Trace : Action<string> with get, set
    abstract Info : Action<string> with get, set
    abstract Warning : Action<string, exn> with get, set
    abstract Error : Action<string, exn> with get, set
    abstract Fatal : Action<string, exn> with get, set
...
type DefaultLogger() = 
    ...
    interface ILogger with
        ...
        member self.Trace 
            with get () = self.Trace
            and set (x) = self.Trace <- x
        
        member self.Info 
            with get () = self.Info
            and set (x) = self.Info <- x
        
        member self.Warning 
            with get () = self.Warning
            and set (x) = self.Warning <- x
        
        member self.Error 
            with get () = self.Error
            and set (x) = self.Error <- x
        
        member self.Fatal 
            with get () = self.Fatal
            and set (x) = self.Fatal <- x
